...
describe('updateTask', () => {
    it('should update the task with new data', async () => {
        ...
        jest.spyOn(taskService, 'processImage').mockImplementation(async (path: string) => [{resolution: 100, path: '/output/image1/1024/f322b730b2322387da77e1c519c7ffef4fc2.jpg'}]);
        const getTask = jest.spyOn(taskRepository, 'getTaskById').mockImplementation(async (taskId: string) => TASK_EXAMPLE);
        const updateTask = jest.spyOn(taskRepository, 'updateTask').mockImplementation(async (taskId: string,task:Task) => TASK_EXAMPLE);
