...
async function downloadImage(url: string, downloadPath: string): Promise<string | null> {
  try {
...
export class TaskService {
  async processImage(image: string): Promise<{ resolution: number; path: string }[]> {
    ...
    if (image.startsWith('http://') || image.startsWith('https://')) {
      ...
      const fileName = generateMd5Hash(image) + fileExtension;
      ...
      const downloaded = await downloadImage(image, localImagePath);
      if (!downloaded) {
        return [];
      }
    }
    ...
    try {
      ...
      const md5Hash = generateMd5Hash(localImagePath);
