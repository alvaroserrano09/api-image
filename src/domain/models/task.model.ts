...
static create(path:string,status: 'pending' | 'completed' | 'failed'):Task{
    return new Task({
        price: Math.floor(Math.random() * 50), images:[{ resolution: 1080, path }] , status:status, taskId:randomUUID(),originalPath:path});
}
